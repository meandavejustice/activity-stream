...
const BATCH_SIZE = 10000; // Set a batch size for history processing.
...
function ActivityStreams(metadataStore, options = {}) {
  ...
  this._setupButton();
  this._initHistoryPreprocessing();
...
ActivityStreams.prototype = {
  ...
  _initHistoryPreprocessing: Task.async(function*() {
    let worker = new ChromeWorker(data.url("ClusteredW2V.js"));
    let historySize = yield PlacesProvider.links.getVisitsCount();

    for (let i = 0; i < historySize / BATCH_SIZE + 1; i++) {
      let links = yield PlacesProvider.links.processHistory({limit: BATCH_SIZE, offset: i * BATCH_SIZE});

      worker.postMessage({
        command: "processHistoryBatch",
        payload: {links}
      });
    }
  }),
