...
Links.prototype = {
  ...
  processHistory: Task.async(function*(options = {}) {
    let {limit, offset, ignoreBlocked} = options;
    if (!limit || limit.options > LINKS_QUERY_LIMIT) {
      limit = LINKS_QUERY_LIMIT;
    }
    ...
    let sqlQuery = `SELECT subquery.visit_date,
                            subquery.url,
                            subquery.title,
                            f.data AS favicon,
                            f.mime_type as mimeType
                    FROM
                      (SELECT DISTINCT h.visit_date, p.url, p.title, p.favicon_id
                      FROM moz_historyvisits AS h
                      LEFT JOIN moz_places AS p
                        ON h.place_id = p.id
                      WHERE p.rev_host NOT IN (${REV_HOST_BLACKLIST})
                      AND p.url NOT IN (${blockedURLs})
                      ORDER BY visit_date) AS subquery
                    LEFT JOIN moz_favicons AS f
                    ON subquery.favicon_id = f.id
                    ORDER BY subquery.visit_date DESC
                    LIMIT :limit
                    OFFSET :offset`;

    let links = yield this.executePlacesQuery(sqlQuery, {
      columns: ["visit_date", "url", "title", "favicon", "mimeType"],
      params: {limit, offset}
    });
   ...
   *
   * @returns {Promise} Returns a promise with the count of moz_historyvisists records
   */
  getVisitsCount: Task.async(function*(options = {}) {
    let {ignoreBlocked} = options;
    let blockedURLs = ignoreBlocked ? [] : this.blockedURLs.items().map(item => `"${item}"`);

    let sqlQuery = `SELECT count(1) as count
                    FROM moz_historyvisits
                    WHERE url NOT IN (${blockedURLs})`;
